...
<body>
  ...
  <section class="main">
    ...
    if (isset($_GET["id"])) {
      $id = $_GET["id"];
      $view2 = mysqli_query($conn, "SELECT * FROM user WHERE user_id = '$id'");
    ...
    <div class='container'>
      <div class='card mx-auto my-3 border rounded'>
        <div class='card-header rounded'>User Details</div>
          <img class='card-img-top rounded my-1 mx-auto d-block rounded-circle' src='uploads/" . $row4["img"] . "' style='width: 25%;' onerror='this.onerror=null; this.src=" . $_SESSION["dp"] . "'  alt='profile picture'>
          <div class='card-body'>
            <table class='table'>
            <tbody>
      ...
      if (mysqli_num_rows($req) > 0) {
        ...
        if ((($req1["sender"] == $myid) && ($req1["receiver"] == $id)) && $req1["status"] == 0) {
          ...
          echo '<a href="function_db.php?sender=' . $row4["user_id"] . '" class="btn text-white btn-sm btn-success">Accept</a>';
        } else if (($req1["receiver"] == $myid or $req1["sender"] == $myid) && $req1["status"] == 1) {
          echo '<a class="btn btn-sm btn-success text-white">Connected</a>';
        }
      } else {
        echo '<a href="function_db.php?receiver=' . $row4["user_id"] . '" class="btn text-white btn-sm btn-success">send request</a>';
      }

      echo " 
           <a href='function_db.php?matu_id=" . $row4["user_id"] . "' class='text-white btn ml-2 btn-primary'>Scores</a>
