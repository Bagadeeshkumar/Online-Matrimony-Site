...
if (isset($_POST['reg_submit'])) {
    ...
    if ($chk_user == 'admin') {
        header(
            'Location: registrationpage.php?error=Invalid username you cannot use the username as admin!'
        );
    } elseif (
        !preg_match(
        ...
        mysqli_num_rows(
            mysqli_query(
                $conn,
                "SELECT * FROM user WHERE username='$chk_user' OR email='$chk_mail'"
            )
        ) > 0
    ) {
...
if (isset($_POST['log_submit'])) {
    ...
    $log_run = mysqli_query(
        $conn,
        "SELECT * FROM user WHERE (username='$log_user' OR email='$log_user') AND (password='$log_pass')"
    );
    if (mysqli_num_rows($log_run) == 1) {
        ...
        if (
            $_SESSION['log'] == 'admin' or
            $_SESSION['log'] == 'matrimonybka@gmail.com'
        ) {
...
if (isset($_POST['info_sub'])) {
    ...
    $_SESSION['nationality'] = mysqli_real_escape_string(
        $conn,
        $_POST['nation']
    );
    ...
    $_SESSION['physical'] = mysqli_real_escape_string(
        $conn,
        $_POST['p_status']
    );
    ...
    $_SESSION['employment'] = mysqli_real_escape_string(
        $conn,
        $_POST['employ']
    );
    $_SESSION['profession'] = mysqli_real_escape_string(
        $conn,
        $_POST['profession']
    );
    $_SESSION['annual'] = mysqli_real_escape_string($conn, $_POST['income']);
    $_SESSION['job_company'] = mysqli_real_escape_string(
        $conn,
        $_POST['company']
    );
    $_SESSION['job_design'] = mysqli_real_escape_string(
        $conn,
        $_POST['position']
    );
    ...
    $_SESSION['pm_status'] = mysqli_real_escape_string(
        $conn,
        $_POST['p_status']
    );
    ...
    $_SESSION['p_profession'] = mysqli_real_escape_string(
        $conn,
        $_POST['p_profession']
    );
...
if (isset($_POST['pre_user'])) {
    ...
    if ($reg_run) {
        ...
        $run6 = mysqli_query(
            $conn,
            "SELECT * FROM user WHERE email='$chk_mail'"
        );
    ...
    if ($chkpass['password'] == $oldpass) {
        if ($newpass == $con_pass) {
            $upadpass = mysqli_query(
                $conn,
                "UPDATE user SET password= '$newpass' WHERE user_id='$id'"
            );
            if ($upadpass) {
                header(
                    'Location: profile.php?success_chg=Password Successfully Changed!'
                );
            }
        } else {
            header(
                'Location: profile.php?error_chg=Password mismatched for new and current password!'
            );
        }
    } else {
...
if (isset($_POST['chg_adsubmit'])) {
    ...
    if ($chkpass['password'] == $oldpass) {
        if ($newpass == $con_pass) {
            ...
            if ($upadpass) {
                header(
                    'Location: settings.php?success_chg=Password Successfully Changed!'
                );
            }
        } else {
            header(
                'Location: settings.php?error_chg=Password mismatched for new and current password!'
            );
        }
    } else {
...
if (isset($_GET['del_id'])) {
    ...
    if ($del_sql) {
        header(
            'Location: admin.php?msg=User removed from database successfully!'
        );
    } else {
...
function matches($myid, $id)
{
    $count = 0;
    require 'db.php';
    $row18 = mysqli_fetch_assoc(
        mysqli_query($conn, "SELECT * FROM user WHERE user_id = $myid")
    );
    $row19 = mysqli_fetch_assoc(
        mysqli_query($conn, "SELECT * FROM user WHERE user_id = $id")
    );
...
if (isset($_GET['up'])) {
    ...
    $p = 'P';
    if (
        mysqli_query($conn, "UPDATE user SET status = '$p' WHERE user_id = $up")
    ) {
        $_SESSION['my_status'] = 'P';
...
if (isset($_GET['receiver'])) {
    ...
    $sent = mysqli_query(
        $conn,
        "INSERT INTO friend(sender, receiver, status) VALUES('$my_id','$to_id',0)"
    );
    if ($sent) {
...
if (isset($_GET['sender'])) {
    ...
    $confirm = mysqli_query(
        $conn,
        "UPDATE friend SET status = 1 WHERE (sender = $from_id AND receiver = $myid)"
    );
    if ($confirm) {
...
if (isset($_GET['del'])) {
    $from_id = $_GET['del'];
    $myid = $_SESSION['my_id'];
    $confirm_delreq = mysqli_query(
        $conn,
        "DELETE FROM friend WHERE ((sender = $from_id AND receiver = $myid) AND status = 0)"
    );
    if ($confirm_delreq) {
...
if (isset($_GET['c_req'])) {
    ...
    $confirm_delreq = mysqli_query(
        $conn,
        "DELETE FROM friend WHERE ((sender = $myid AND receiver = $to_id) AND status = 0)"
    );
    if ($confirm_delreq) {
...
if (isset($_GET['rem'])) {
    $id = $_GET['rem'];
    $myid = $_SESSION['my_id'];
    $confirm_delcon = mysqli_query(
        $conn,
        "DELETE FROM friend WHERE (((sender = $myid AND receiver = $id) OR (sender = $id AND receiver = $myid)) AND status = 1)"
    );
    if ($confirm_delcon) {
